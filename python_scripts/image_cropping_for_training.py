...
def detect_face_of_block(img, testing_param):
...
def do_face_detection_and_cropping_for_all_image(TESTING):
    def do_face_detection_and_cropping_for_img(label_name, img, img_name):
        try:
            # face_detection
            [min_width, min_height, max_width, max_height] = detect_face_of_block(img, testing_param=TESTING)
            img_2 = img.copy()
            final_image = cv.rectangle(img_2, (min_width, min_height), (max_width, max_height), (0, 255, 255), 5)
            ...
            cv.imwrite('TEMP_DO_NOT_TOUCH/photos after cutting and resizing/' + label_name + '/' + img_name, crop_img)
    ...
    # remove previous files at 'TEMP_DO_NOT_TOUCH/photos with bounding box' and 'photos after cutting and resizing'
    path_to_delete = 'TEMP_DO_NOT_TOUCH/photos with bounding box'
    for filename in os.listdir(path_to_delete):
        file_path = os.path.join(path_to_delete, filename)
        try:
    ...
    path_to_delete = 'TEMP_DO_NOT_TOUCH/photos after cutting and resizing'
    for filename in os.listdir(path_to_delete):
        file_path = os.path.join(path_to_delete, filename)
        try:
            if os.path.isfile(file_path) or os.path.islink(file_path):
    ...
    images_dir = "../Images"
    # create label folders for images in 'photos after cutting and resizing'
    for filename in os.listdir(images_dir):
        try:
            os.makedirs("TEMP_DO_NOT_TOUCH/photos after cutting and resizing" + "/" + filename)
        except FileExistsError:
            pass

    for filename_ in os.listdir(images_dir):
        for potential_photos in os.listdir(images_dir + "/" + filename_):
            img_ = cv.imread(images_dir + "/" + filename_ + "/" + potential_photos)
            if img_ is not None:
                do_face_detection_and_cropping_for_img(filename_, img_, potential_photos)
            else:
                print(filename_ + " is not an image, skipped")


do_face_detection_and_cropping_for_all_image(False)
